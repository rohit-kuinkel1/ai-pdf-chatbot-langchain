...
async function setupSupabase() {
  ...
  try {
    ...
    if (error) {
      ...
      const { error: createError } = await supabaseClient.rpc('pg_execute', { query: sql });
  ...
  try {
    ...
    if (collections.length === 0) {
      ...
      try {
        ...
        console.warn('Warning: Could not create vector index. This is expected for local MongoDB (not Atlas).');
        console.warn('If you are using MongoDB Atlas, check your connection string and permissions.');
      }
...
async function main() {
  ...
  const setupSupabaseFlag = process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY;
  const setupPostgresFlag = process.env.POSTGRES_HOST || process.env.POSTGRES_USER;
  const setupMongoDBFlag = process.env.MONGODB_URI;
  
  if (!setupSupabaseFlag && !setupPostgresFlag && !setupMongoDBFlag) {
    console.log('No database configuration found in .env. Please configure at least one database.');
    process.exit(1);
  }
  ...
  if (allSuccess) {
    ...
    console.log('You can now use the PDF Chatbot with your preferred database.');
  } else {
